...
class ByteLevelBPETokenizer:
    """
    Experimental v2 byte-level BPE tokenizer.

    Extends the educational BPE implementation with an incremental pair
    statistics approach intended to reduce repeated full-corpus scanning
    during training.

    This version is an optimization experiment for learning purposes and
    is not necessarily faster than the simpler implementation.
    """
    ...
    def _build_pair_stats(self, corpus):
        pair_counts = Counter()
        pair_to_sequences = defaultdict(set)

        for seq_id, tokens in enumerate(corpus):
            local_pairs = Counter(zip(tokens, tokens[1:]))
    ...
    def _remove_sequence_stats(
        ...
        for pair, count in local_pairs.items():
            pair_counts[pair] -= count
            pair_to_sequences[pair].discard(seq_id)
    ...
    def train(self, texts, vocab_size):
        ...
        pair_counts, pair_to_sequences = self._build_pair_stats(corpus)
        ...
        for rank in range(max_merges):
            ...
            affected_sequences = list(pair_to_sequences[pair])

            for seq_id in affected_sequences:
                old_tokens = corpus[seq_id]

                self._remove_sequence_stats(
                    seq_id,
                    old_tokens,
                    pair_counts,
                    pair_to_sequences
                ...
                new_tokens = self.merge_pair(
                    old_tokens,
                    pair,
                    new_token_id
                )

                corpus[seq_id] = new_tokens

                self._add_sequence_stats(
                    seq_id,
                    new_tokens,
                    pair_counts,
                    pair_to_sequences
